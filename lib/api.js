...
// Required by Stage 3 for all protected profile endpoints.
const API_HEADERS = {
...
async function tryRefresh(creds) {
  // Refresh rotation: backend invalidates old refresh and returns a new pair.
  const api = getApiUrl();
...
async function ensureFreshCredentials() {
  ...
  const skew = 15 * 1000;
  // Refresh slightly before expiry to reduce request-time failures.
...
async function apiFetch(path, options = {}, didRefresh = false) {
  ...
  // One retry path after refresh keeps command UX smooth.
